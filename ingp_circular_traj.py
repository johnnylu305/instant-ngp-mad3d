...
if __name__=="__main__":
    ...
    parser.add_argument("--src", required=True, help="Source of Json File")
    parser.add_argument("--dst", required=True, help="Destination for a new Json File")
    parser.add_argument("--num_frames", type=int, default=150)
    parser.add_argument("--r", type=float, default=1.5)
    parser.add_argument("--cur_h", type=float, default=0.5)
    parser.add_argument("--center", nargs=3, default=[0.35, 0.52, 0.5], type=float)
    ...
    # radius of circle
    r = args.r
    # one circle every t frames
    angle_step = 2*math.pi/t
    cur_angle = 0
    cur_h = args.cur_h #3.7
    ...
    for i in range(t):
        ...
        if 'fl_x' in params:
            ...
            trajectory['fl_y'] = params['fl_y']*(h/params['h'])
            trajectory['cx'] = params['cx']*(w/params['w'])
            trajectory['cy'] = params['cy']*(h/params['h'])
            trajectory['w'] = w
            trajectory['h'] = h
            trajectory['camera_angle_x'] = 2*np.arctan2(params['w'],2*params['fl_x'])
            trajectory['camera_angle_y'] = 2*np.arctan2(params['h'],2*params['fl_y'])
        else:
            trajectory['frames'][-1]['fl_x'] = params['frames'][0]['fl_x']*(w/params['frames'][0]['w'])
            trajectory['frames'][-1]['fl_y'] = params['frames'][0]['fl_y']*(h/params['frames'][0]['h'])
            trajectory['frames'][-1]['cx'] = params['frames'][0]['cx']*(w/params['frames'][0]['w'])
            trajectory['frames'][-1]['cy'] = params['frames'][0]['cy']*(h/params['frames'][0]['h'])
            trajectory['frames'][-1]['w'] = w
            trajectory['frames'][-1]['h'] = h
            trajectory['frames'][-1]['camera_angle_x'] = 2*np.arctan2(params['frames'][0]['w'],2*params['frames'][0]['fl_x'])
            trajectory['frames'][-1]['camera_angle_y'] = 2*np.arctan2(params['frames'][0]['h'],2*params['frames'][0]['fl_y'])           
